import { signInAction } from "@/action/oauth";

interface SignInButtonProps {
	btnText: string;
}

function SignInButton({ btnText }: SignInButtonProps) {
	return (
		<form action={signInAction}>
			<button
				type="submit"
				className="flex items-center gap-6 text-sm border border-border px-8 py-3 rounded-xl hover:bg-muted/70 transition-all"
			>
				{/* biome-ignore lint/a11y/noSvgWithoutTitle: No need for this */}
				<svg
